...
> projects — `spider_registries` on **vault** (`dknahxavnrtaqlatflot`, 49 rows, last
> written 2026-06-07) and `branch_registry` on **runtime** (`ruvwundetxnzesrbkdzr`,
...
export SUPABASE_URL=<runtime url>            # supabase/URL  (ruvwundetxnzesrbkdzr)
...
```bash
cd ~/Dev/spidersan && supabase db push --linked   # repo links ruvwundetxnzesrbkdzr
# or apply 20260219_spider_registries.sql via the mgmt API against ruvwundetxnzesrbkdzr
...
```bash
# apply 20260623_harden_spidersan_rls.sql against ruvwundetxnzesrbkdzr, then:
...
# (dknahxavnrtaqlatflot) — drops the orphaned table + views.
```

### 6. Update the project maps (so this can't drift back)
- `~/Dev/treebird/agents/mycsan/knowledge/cookbook/supabase-projects.md`
