...
function summarizeBranchConflicts(conflicts: BranchConflict[]): { tier1: number; tier2: number; tier3: number } {
    ...
    // Replaced multiple O(N) `.filter(condition).length` calls with a single
    // manual counting loop. This avoids redundant traversals and intermediate array allocations.
