...
export function renderPulseReport(opts: PulseRenderOptions): string {
    ...
    if (opts.conflicts.length === 0) {
        ...
        // ⚡ Bolt: Use a single loop to count tiers instead of multiple .filter().length
        // to prevent redundant O(N) memory allocations and array traversals
        let tier3 = 0;
        let tier2 = 0;
        let tier1 = 0;
