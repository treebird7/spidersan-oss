...
export async function buildContext(options: BuildContextOptions = {}): Promise<SpiderContext> {
  ...
  // Performance Optimization: Consolidating multiple independent array.filter(condition).length
  // calls into a single for...of loop avoids three redundant O(N) traversals
