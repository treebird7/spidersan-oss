...
export async function buildContext(options: BuildContextOptions = {}): Promise<SpiderContext> {
  ...
  // ⚡ Bolt: Consolidate multiple independent array.filter(condition).length calls into a single loop
  // to track counts manually, avoiding redundant O(N) traversals and intermediate array allocations.
  let tier1Count = 0;
  let tier2Count = 0;
  let tier3Count = 0;
