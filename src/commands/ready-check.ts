...
function getChangedFiles(): string[] {
    try {
        let diff = '';
        try {
            diff = execFileSync('git', ['diff', '--name-only', 'main...HEAD'], { encoding: 'utf-8', stdio: 'pipe' });
        } catch {
            try {
                diff = execFileSync('git', ['diff', '--name-only', 'HEAD~1'], { encoding: 'utf-8', stdio: 'pipe' });
            } catch {
                try {
                    diff = execFileSync('git', ['diff', '--name-only', 'HEAD'], { encoding: 'utf-8', stdio: 'pipe' });
                } catch {
                    diff = '';
                }
            }
