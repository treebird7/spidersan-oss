...
export function shouldExcludeFile(file: string, compiledPatterns: Minimatch[]): boolean {
    return compiledPatterns.some(pattern => pattern.match(file));
}
...
export const readyCheckCommand = new Command('ready-check')
    ...
    .action(async (options) => {
        ...
        if (config.readyCheck.enableWipDetection && !options.skipWip) {
            ...
            // Performance Optimization: Pre-compile Minimatch instances
