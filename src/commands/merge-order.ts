...
function buildConflictGraph(branches: Branch[]): Map<string, string[]> {
    ...
    // Performance Optimization: Use an inverted index to map files to the branches that touch them
    // Reduces time complexity from O(N^2 * M) to O(N * M) by avoiding N^2 branch-to-branch comparisons
