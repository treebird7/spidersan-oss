...
export const doctorCommand = new Command('doctor')
    ...
    .action(async (options) => {
        if (options.remote) {
            ...
            // Replace multiple .filter(condition).length array allocations
            // with a manual O(N) loop to tally states efficiently.
        ...
        let warnings = 0;
        // ⚡ Bolt: Performance Optimization
        // Tally results during a single pass instead of making multiple calls
        // to .filter().length, saving redundant O(N) traversals.
