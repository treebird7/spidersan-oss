...
export const doctorCommand = new Command('doctor')
    ...
    .action(async (options) => {
        if (options.remote) {
            ...
            // Performance Optimization: Consolidate multiple independent array.filter(condition).length calls
            // into a single loop tracking counts manually. This avoids redundant O(N) traversals
        ...
        // Performance Optimization: Replace multiple .filter(condition).length calls with a single loop
