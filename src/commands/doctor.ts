...
export const doctorCommand = new Command('doctor')
    ...
    .action(async (options) => {
        if (options.remote) {
            ...
            // Performance Optimization: Consolidate multiple filter.length calls into a single loop
            // to avoid O(N) intermediate array allocations and redundant traversals.
            let pushNeeded = 0;
            let pullNeeded = 0;
            let mergeNeeded = 0;
            let upToDate = 0;
