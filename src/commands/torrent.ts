...
torrentCommand
    ...
    .action(async (options: { json?: boolean }) => {
        ...
        // Build conflict graph using inverted index (O(N*M) vs O(N²))
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        ...
        // Performance Optimization: Consolidate multiple .filter().length checks into a single
        // loop to avoid redundant O(N) array allocations and traversals.
        let completed = 0;
        let active = 0;
        ...
        let childrenWithZero = 0;
        for (const r of roots) {
            if (r.children.length === 0) childrenWithZero++;
        }
        const children = total - childrenWithZero;
