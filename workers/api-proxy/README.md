...
# Create KV namespace (wrangler v4 syntax — spaces not colons)
...
```bash
# Add a key (wrangler v4 syntax)
wrangler kv key put --binding=VALID_API_KEYS \
  "sk-spidersan-abc123" \
  '{"tier":"free","created":"2026-01-01T00:00:00Z","label":"user@example.com"}'

# Add a contributor key (unlimited)
wrangler kv key put --binding=VALID_API_KEYS \
  "sk-spidersan-contrib-xyz" \
  '{"tier":"contributor","created":"2026-01-01T00:00:00Z","label":"user@example.com"}'
...
- Free tier: 50 req/day (advisory KV counter, daily bucket, eventually consistent)
- Contributor tier: unlimited (rate limit check skipped)
- `X-RateLimit-Limit` and `X-RateLimit-Remaining` headers on free-tier responses
