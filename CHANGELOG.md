...
- **A graphify knowledge graph of the repo, regenerated on demand into the now-gitignored `graphify-out/`.** ~1,980 nodes and ~4,280 edges over 247 code files, queryable with `graphify explain` / `affected` / `path` instead of grepping; `AGENTS.md` carries the commands, each run verbatim first. Nothing is committed — it rebuilds in ~30s. Extraction is local tree-sitter AST under `--code-only`, with the wrapper running under `env -i` because `extract` picks its backend from *whichever API key is set*: the stripped environment is the control, the flag is only the intent. `--with tree-sitter-sql` fails silently without the grammar, parsing the 26 `.sql` files to nothing behind a single warning and exit 0. The freshness check is `git rev-list --count HEAD..origin/main` rather than an mtime comparison, because a checkout behind the remote rebuilds into a graph that *looks* fresh — `built_at_commit` agrees with `HEAD` while both are stale. Measured on this repo: every file parsed with no syntax errors, `path` returns meaningful short routes (`conflictsCommand` → `fetchCrossMachineBranches()` → `SupabaseStorage`), `god-nodes` mixes framework fan-in (`vitest` 67, `commander` 45) with real hubs (`getStorage()` 63, `Branch` 54), and 176 labels are shared by more than one node — which matters because the verbs disagree about ambiguity: `explain` refuses and lists the candidate ids, `affected` refuses with *No unique node match*, and `path` does neither, answering *No directed path found* exactly as it would for a genuine negative.

### Fixed

- **`conflicts --real` merges into the REMOTE trunk, not a stale local one** (sp-vifo) — the base came from `getTrunkBranch()`, which returns a branch *name* (`main`), so merge-tree ran against whatever the local ref happened to point at. In a long-lived checkout that drifts: on `~/Dev/treebird` a leftover worktree from the retired beads tool had pinned local `main` **78 commits** behind `origin/main`, and `--real` reported *clean vs main* on a branch that `--real --base origin/main` showed conflicting. Green, earned by comparing against the wrong thing. The base is now `origin/<trunk>` when that remote-tracking ref exists (via the existing `resolveBranchRef()`), falling back to the local name when it does not — fresh clones and no-origin repos are unchanged. **No fetch**: this reads refs already on disk, because `--real` runs from a pre-merge hook and a check that hits the network every invocation is a different problem; `origin/<trunk>` can therefore be stale too, which is why the report always names the base it used. An explicit `--base` still short-circuits trunk detection entirely. The `--pr` arm already reasoned this way (tb-57fr) and is unchanged — it fetches, because it is already fetching the PR head.
- **`conflicts` no longer claims merge-readiness from a registry-only check** (PR #295) — the zero-conflict path printed "✅ You're good to merge!" after comparing *registered file lists*, which runs no merge and is structurally blind to add/add and content conflicts against the base. Repro'd live: the pre-merge hook printed the all-clear while `git merge-tree` reported `CONFLICT (add/add)`. Both emitters (the command, and the unwired `renderConflictReport` seam) now state scope — `No registry overlap for "<branch>" (vs N registered branches)` plus the command that does answer merge-readiness. The branch count makes "did the work" observable. Guarded by tests on both the live emitter and the seam.
- **`conflicts --real --vs-prs` no longer silently drops the cross-PR half** (sp-a1l3) — `options.real` short-circuited into the merge-tree path and `return`ed, so the `--vs-prs` block was never reached. `conflicts --pr <N> --real --vs-prs` — documented as THE authoritative pre-merge check — therefore printed only "clean vs trunk" and exited 0 while never consulting GitHub for other open PRs: a check reporting success while doing nothing. The two flags now compose: the merge-tree check runs first, then execution falls through to the cross-PR/overlap pass, and the process exit code is `max(real, overlap)` (real conflicts are tier-blocking, overlap only blocks under `--strict`). `--real` alone, `--pr N` alone and `--vs-prs` alone are unchanged and pinned by tests. On the composed path an uninitialized registry or an unregistered branch degrades the overlap half with a loud "did NOT run" warning instead of failing the whole command, so `--real`'s registry-independence survives. Proven by effect with a `gh`-disabled shim: before, `--real --vs-prs` printed one line and never called `gh`; after, it prints the merge-tree verdict *and* the cross-PR/registry overlap report.
...
- **`branch_registry` uniqueness is now composite `(repo_name, branch_name)`** (tb-31t4, PR #282 + #284) — the fleet-wide auto-register rollout proved the old global-UNIQUE `branch_name` collision: the same branch name pushed from three repos left one row (first writer wins). Interim function fix (#282): the files-refresh PATCH is scoped by `repo_name` and a colliding insert reports `collided: true` instead of silently touching another repo's row. Real fix (#284, applied to prod as migration `20260801195726` with `UNIQUE NULLS NOT DISTINCT` — legacy NULL-`repo_name` rows keep their shared-namespace semantics): cross-repo same-name branches now coexist; same-repo duplicates still violate. CLI unaffected — its only `branch_registry` touch is a connectivity probe; machine sync uses `spider_registries` (already composite).
- **eslint 8 → 10, flat config** (tb-wb47, PR #283) — `eslint.config.mjs` replaces `.eslintrc.json`, same rule intent. The two new recommended rules flagged 8 real issues, fixed in code not config: three dead initializers removed (`no-useless-assignment`) and five `throw` sites now attach `{ cause }` (`preserve-caught-error`).
...
- **CI dependency audit scoped to production deps** (`npm audit --omit=dev --audit-level=high`) — GHSA-mh99-v99m-4gvg (brace-expansion, published 2026-07-29) is unfixable inside eslint 8's transitive tree: the only patched release (5.0.8) breaks minimatch@3's direct-function import (verified). The published CLI ships none of that chain. Full-tree audit returns with the eslint 8→10 flat-config migration (tracked as invoak tb-wb47). js-yaml and postcss high advisories fixed via lockfile bump in the same pass.
  - **Restored to full tree** in PR #283: with eslint 10 the dev-chain advisories are gone — `npm audit --audit-level=high` runs unscoped again and reports 0 vulnerabilities.

### Removed

- **The Treebird Hub transport is gone** (sp-hnjf, follow-up to sp-8iqm/#290) — `hub.treebird.uk` was decommissioned 2026-09-05 and answers `404 Application not found` behind `x-railway-fallback: true`, so every call site below was dead code, not a severed channel. Deleted `src/lib/hub.ts` (`HubClient` + `FetchHubAdapter`), the `HUB_URL` env var, and the `--hub` / `--hub-sync` flags on `watch`, `auto start` and `pulse`, plus `autoWatch.hub` / `autoWatch.hubSync` in `.spidersanrc`. An existing config file carrying those keys stays valid and inert — `validateConfig` only checks keys it knows. `socket.io-client` dropped with the `--hub` WebSocket.
  - Three further **unconditional success lines** of the same class as `git-events-subscriber.ts:467` are gone: `watch.ts` printed `📤 Posted conflict to Hub chat` after awaiting a `Promise<void>` that swallowed failure at *both* the adapter and client layers; `torrent claim` / `torrent complete` each printed `✅ Notified Hub` from inside a `try` that only catches throws, so a 404 response read as success; and `conflicts --wake` printed `✅ Woke N agent(s)` for a helper that returned `true` unconditionally. `--wake` / `--retry` are removed outright, and `conflicts --auto` with them — it only ever suppressed the wake-flow confirmations and is now read by nothing (`register --auto` is a different flag and is untouched).
  - **Re-pointed, not dropped:** the `watch` conflict alert and `conflicts --notify` now go through `deliver()`/`notify()` from `src/lib/notify.ts`, the seam landed in #290 — so they report the real delivery outcome and never claim a send they did not inspect. Delivery is configured by `SPIDERSAN_ROOM_TOKEN`, not by a flag; unset means the alert is logged `NOT NOTIFIED`.
  - **Dropped, not re-pointed:** remote-drift alerts (`pulse --hub-sync`, and the `watch --fetch-poll` variant). Drift is not a conflict, so it does not belong in the seam's event class, and both commands already print the drift report the operator is reading.
