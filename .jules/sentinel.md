...
## 2026-02-18 - Ineffective Validation due to Ignored Return Value
**Vulnerability:** The `validateRegistrationFiles` function called `sanitizeFilePaths` (which returns a sanitized array) but ignored the return value, meaning invalid files were not actually filtered out.
**Learning:** Security checks that rely on "sanitization" functions must use the returned value. If a function is intended to validate (and abort), it must throw an exception.
**Prevention:** Design validation functions to throw errors on invalid input rather than silently filtering, especially for critical security checks.
